...
export class InicioPage implements OnInit {
  ...
  async mostrarHistorial() {
    const modal = await this.modalController.create({
      ...
      componentProps: { usuarioId: this.nombreUsuario }, // Pasar datos al componente modal.
    });

    await modal.present();

    const { data } = await modal.onWillDismiss(); // Manejo de los datos retornados del modal.
    if (data) {
      console.log('Datos retornados del historial:', data);
    }
  }
  ...
  async iniciarEscaneo() {
    try {
      ...
      const qrPattern = /^([A-Z0-9]+)\|([A-Z0-9]+)\|([A-Z0-9]+)\|(\d{8})$/;
  ...
  async guardarAsistencia() {
    // Código para guardar asistencia...
  }
